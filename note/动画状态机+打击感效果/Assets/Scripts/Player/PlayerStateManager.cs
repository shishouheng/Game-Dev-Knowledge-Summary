...
public class PlayerStateManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        AddState<PlayerStateAttack01>();
        AddState<PlayerStateAttack02>();
    }
    ...
    public void ChangeState<T>(PlayerState nextState)where T:PlayerStateBase
    {
        //如果当前状态不为空，则执行状态的退出方法
        if (currentPlayerState != null)
            currentPlayerState.OnExit();
        //将状态切换为下一个状态并执行进入该状态的方法
        currentPlayerState = states[typeof(T)];
        currentPlayerState.OnEnter(nextState);
    }

    //将PlayerState转换为PlayerStateBase
    public PlayerStateBase GetPlayerStateBaseByPlayerState(PlayerState state)
    {
        foreach (PlayerStateBase playerStateBase in states.Values)
        {
            if (playerStateBase.currentState == state)
